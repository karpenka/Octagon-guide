function octagon_ps_ab_r2(lambda,varargin)
...
dlmwrite(strjoin({strjoin({'oct_abr','lambda',num2str(lambda),'l'},'_'),'.dat'},''),Il,' ');
...
savefig(strjoin({'oct_abr','lambda',num2str(lambda),'l'},'_'));
figure;
surf(X,Y,Im)
xlabel('a [m]')
ylabel('b [m]')
...
savefig(strjoin({'oct_abr','lambda',num2str(lambda),'m'},'_'));
figure;
surf(X,Y,Is)
xlabel('a [m]')
ylabel('b [m]')
...
savefig(strjoin({'oct_abr','lambda',num2str(lambda),'s'},'_'));