...
h = 0.15;
L0 = 1;
a_min = 0.02; a_max = 0.21; a_step = 0.01;
length_min = 5; length_max = 40; length_step = 5;
ppath = 'D:\JOB\github\Octagon-guide\octagon\try\stl\';

for a = a_min:a_step:a_max
    for length = length_min:length_step:length_max
        try
        
        length_str = sprintfc('%d',length);
        filename = ['l_' length_str{1} '_a_' num2str(a*100) '.stl'];            
        ...
        model.param.set('b', num2str(a/sqrt(2)));
        model.param.set('w', num2str(w));
        model.param.set('h', num2str(h));
        model.param.rename('h', 'height');
        model.param.rename('w', 'width');
        model.param.set('L0', num2str(length));
        model.param.set('L1', num2str(L0));
        model.param.set('L2', num2str(length));
